...
from django.views.decorators.csrf import csrf_exempt # Solo para pruebas. Eliminar en producción y todas sus invocaciones.
...
def historico(request, id):
    data = list(RegistroSensor.objects.filter(boya=id).values('ph','turbidez','temperatura','conductividad', 'fecha_creacion').order_by('-fecha_creacion'))
    if not data:
        return JsonResponse({'success': False, 'message': 'No se encontraron registros para esta boya'}, status=404)
    ...
    try:
        ...
        data = {
            "parametros": {
                ...
                "conductividad": {"valor": registro.conductividad, "score": score_conduc}
            },
            "puntaje_total": score,
            "estado": estado,
...
def scorePh(ph):
    if ph >= 6.5 and ph <= 8.5: return 0
    if ph >= 6.0 and ph <= 9: return 1
    return 2
...
def scoreTemperature(t):
    if t >= 15 and t <= 25: return 0
    if t >= 10 and t <= 30: return 1
    return 2

def diagnostic(score):
    if score <= 2: return "Bueno", "No requiere mantenimiento"
    if score <= 5: return "Regular", "Requiere mantenimiento"
    if score <= 7: return "Malo", "Requiere tratamiento químico"
